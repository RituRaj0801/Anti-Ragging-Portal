...
<title>Set Status Grievance</title>
...
<body background="images/pic8.jpg">
	...
	<div id="mydiv">
		...
		%>
		<a href="login.html">
			<%//session.invalidate();%><input type="button" name="logout"
			value="Logout">
		</a>
	</div>
	</nav> </section>
...
{
	Thread.sleep(5000);
	response.sendRedirect("http://localhost:8080/AntiRagging/searchGrievance.jsp");
}
else
{
try
...
Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/AntiRagging", "root",
		"Ns422444");
PreparedStatement st = conn.prepareStatement("update grev set status=? where grevID=?");
st.setString(1, s);
st.setString(2, (String)session.getAttribute("grevID"));
st.execute();
out.print("<center style='color: white'>The Grievance Status Has been Updated</center>");
}
catch(Exception e)
{
	e.printStackTrace();
